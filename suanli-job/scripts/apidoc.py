#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""suanli-job：Apifox 官方文档实时查询（read-through，仅标准库）。
...
用法：
  ...
  python scripts/apidoc.py get /task/job/create      # 按路径查完整文档
  python scripts/apidoc.py get job-create            # 按短名（api/*.md 文件名）
"""
import argparse
import json
import re
...
FILE_ALIAS = {
    ...
    "job-tasks": ("get", "/api/task/job/search"),
    "job-detail": ("get", "/api/task/job/detail"),
    "job-create": ("post", "/api/task/job/create"),
    "job-stop": ("post", "/api/task/job/stop"),
    "job-queue-list": ("get", "/api/job/queue/group/search"),
    "job-queue-detail": ("get", "/api/job/queue/group/detail"),
}
...
def cmd_list(args):
    ...
    if not flat:
        ...
        return 4
    print(f"suanli-job 相关端点（{len(flat)} 个）：\n")
...
def main():
    ap = argparse.ArgumentParser(
        description="suanli-job Apifox 文档实时查询（read-through）")
    ...
    p.add_argument("key", help="接口路径或短名（如 job-create）")
