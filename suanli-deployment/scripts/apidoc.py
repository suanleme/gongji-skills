#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""suanli-deployment：Apifox 官方文档实时查询（read-through，仅标准库）。
...
用法：
  ...
  python scripts/apidoc.py get /task/deployment/create   # 按路径查完整文档
  python scripts/apidoc.py get task-create          # 按短名（api/*.md 文件名）
"""
import argparse
import json
import re
...
FILE_ALIAS = {
    "resource": ("get", "/api/deployment/resource/search"),
    "object-storage": ("get", "/api/storage/get_storage"),
    "fee-time-query": ("get", "/api/billing/get_billing_record"),
    "fee-task-query": ("get", "/api/billing/get_task_billing_record"),
    "node-list": ("get", "/api/deployment/task/points"),
    "node-log": ("get", "/api/deployment/task/point_log"),
    "node-event": ("get", "/api/deployment/task/pod_event"),
    "node-delete": ("post", "/api/deployment/task/delete_pod"),
    "node-num-edit": ("post", "/api/deployment/task/change_points"),
    "task-list": ("get", "/api/deployment/task/search"),
    "task-detail": ("get", "/api/deployment/task/detail"),
    "task-create": ("post", "/api/deployment/task/create"),
    "task-edit": ("post", "/api/deployment/task/update"),
    "task-puase": ("post", "/api/deployment/task/pause"),
    "task-recover": ("post", "/api/deployment/task/recover"),
    "task-stop": ("post", "/api/deployment/task/stop"),
}
...
def cmd_list(args):
    ...
    if not flat:
        ...
        return 4
    print(f"suanli-deployment 相关端点（{len(flat)} 个）：\n")
...
def main():
    ap = argparse.ArgumentParser(
        description="suanli-deployment Apifox 文档实时查询（read-through）")
    ...
    p.add_argument("key", help="接口路径或短名（如 task-create）")
