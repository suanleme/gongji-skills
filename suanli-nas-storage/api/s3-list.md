...
info:
  ...
  version: 1.0.0
paths:
  /api/storage/nas/v1/s3/list:
    get:
      summary: 查询对象存储互传任务列表
      deprecated: false
      description: >-
        ### **接口说明**
        ...
        分页查询当前租户的对象存储互传任务。count 受 statuses 筛选影响；status_counts 为各状态数量，不受 statuses
        影响，且不含 Deleted。应答不含 AccessKey 和 SecretKey。
      ...
      parameters:
        - name: storage_id
          in: query
          description: 关联卷 ID。不传或小于等于 0 时查询全部任务
          required: false
          example: 1001
          schema:
            type: integer
        ...
        - name: statuses
          ...
          description: >-
            任务状态，多个值用英文逗号分隔。可选值
            Queuing（排队中）、Pending（处理中）、Running（运行中）、Stopped（已停止）、Completed（已完成）、Error（失败）、Exception（异常）、Deleted（已删除）。非法值返回
            S002
          required: false
          example: Running,Exception
          schema:
            type: string
        - name: page
          in: query
      ...
      responses:
        '200':
          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/S3ListEnvelope'
              example:
                code: '0000'
                message: success
                data:
      ...
      x-run-in-apifox: https://app.apifox.com/web/project/3025695/apis/api-502709063-run
components:
  schemas:
    S3ListEnvelope:
      type: object
        ...
        data:
          ...
          properties:
            count:
              ...
              format: int64
              title: 总条数
              description: 受 statuses 和 storage_id 筛选影响
            status_counts:
              $ref: '#/components/schemas/S3StatusCounts'
            results:
              type: array
              title: 当前页数据
              items:
    ...
    S3StatusCounts:
      type: object
      title: 分状态数量
      description: 同一租户下各状态任务数量，不含 Deleted。不受请求 statuses 影响。无任务的状态为 0
      required:
        - pending
        - running
        - stopped
