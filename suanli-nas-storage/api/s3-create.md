...
info:
  ...
  version: 1.0.0
paths:
  /api/storage/nas/v1/encrypt/s3/create:
    post:
      summary: 创建对象存储互传任务
      deprecated: false
      description: >-
        ...
        在指定卷下创建对象存储互传任务。请求体须加密后以 text/plain 提交。关联卷必须为 Active。AccessKey 和
        SecretKey 仅用于本次请求，不会保存。受理成功后 status 为 Queuing。
      ...
      requestBody:
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/S3CreateReq'
            example:
              storage_id: 1001
      ...
      x-run-in-apifox: https://app.apifox.com/web/project/3025695/apis/api-502709065-run
components:
  schemas:
    S3CreateReq:
      type: object
      ...
      properties:
        storage_id:
          ...
          format: int64
          title: 卷 ID
          description: 必须大于 0，且卷状态为 Active
        name:
          type: string
          title: 任务名称
        ...
        ignore_patterns:
          type: array
          title: 忽略规则
          description: 按文件名后缀过滤，如 .tmp、.log。为空时不同步过滤
          items:
            type: string
          nullable: true
      x-apifox-orders:
