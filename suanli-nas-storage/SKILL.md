...
用户需要让 Job / 弹性部署任务读取或写入 NAS 数据时，必须使用平台原生 NAS 挂载：
在任务配置的 `services[].nas_storage_config` 中填写 `storage_id` 和 `target_dir`，
业务程序通过容器内的挂载目录访问文件。

- 不得为了让任务访问 NAS，自动开通 SFTP/SFTPGo，再在 Pod 内通过
  HTTP/WebDAV/SFTP 请求访问数据，或使用 FUSE 客户端模拟挂载。
- 不得为绕过原生挂载而修改业务代码、编写传输 SDK，或把存储访问凭证注入业务程序。
- SFTP/WebDAV 连接用于用户明确要求的文件上传、下载或外部访问；
  获取到这些连接信息，不代表应将其作为任务运行时的数据访问方式。
- 原生挂载失败时，检查卷状态、地域匹配、storage_id、target_dir 和任务配置，
  报告具体原因，不自动改走 SFTP/WebDAV。
- 用户明确要求在 Pod 内使用传输协议时，先说明原生挂载方案并澄清用途，
  不把文件传输需求自行解释为任务挂载方案。
