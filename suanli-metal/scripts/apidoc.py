#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""suanli-metal：Apifox 官方文档实时查询（read-through，仅标准库）。
...
用法：
  ...
  python scripts/apidoc.py get /output/v2/device_order/buy_v2   # 按路径查完整文档
  python scripts/apidoc.py get order-create         # 按短名（api/*.md 文件名）
"""
import argparse
import json
import re
...
FILE_ALIAS = {
    "single-product-list": ("post", "/api/output/v2/device-output/page_list_product_single_v2"),
    "group-product-list": ("post", "/api/output/v2/device-output/page_list_product_network_v2"),
    "order-create": ("post", "/api/output/v2/device_order/buy_v2"),
    "order-list": ("post", "/api/output/v2/device_order/get_order_list_v2"),
    "rent-single-device-list": ("post", "/api/output/v2/device-output/list_rent_device_single_v2"),
    "rent-group-device-list": ("post", "/api/output/v2/device-output/list_rent_device_network_v2"),
    "device-detail": ("post", "/api/output/v2/device_order/get_device_details_v2"),
    "auto-renew": ("post", "/api/output/v2/auto_renew_device_config/set_auto_renew_config"),
    "cancel-auto-renew": ("post", "/api/output/v2/auto_renew_device_config/delete_auto_renew_config"),
}
...
def cmd_list(args):
    ...
    if not flat:
        ...
        return 4
    print(f"suanli-metal 相关端点（{len(flat)} 个）：\n")
...
def main():
    ap = argparse.ArgumentParser(
        description="suanli-metal Apifox 文档实时查询（read-through）")
    ...
    p.add_argument("key", help="接口路径或短名（如 order-create）")
