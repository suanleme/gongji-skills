#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""suanli-image-preheat：Apifox 官方文档实时查询（read-through，仅标准库）。
...
用法：
  ...
  python scripts/apidoc.py search "预热"             # 按关键词搜端点
  python scripts/apidoc.py get /task/image_preheat/create   # 按路径查完整文档
  python scripts/apidoc.py get image-preheat-create  # 按短名（api/*.md 文件名）
"""
import argparse
import json
import re
...
SKILL_PREFIXES = [
    "/api/task/image_preheat/",
]

# api/*.md 文件名（去 .md）→ (method, path)；get 短名匹配用
FILE_ALIAS = {
    "region-query": ("get", "/api/task/image_preheat/get_regions"),
    "image-preheat-list": ("get", "/api/task/image_preheat/search"),
    "image-preheat-detail": ("get", "/api/task/image_preheat/detail"),
    "image-preheat-create": ("post", "/api/task/image_preheat/create"),
    "image-preheat-update": ("post", "/api/task/image_preheat/update"),
    "image-preheat-stop": ("post", "/api/task/image_preheat/stop"),
}
...
def cmd_list(args):
    ...
    if not flat:
        ...
        return 4
    print(f"suanli-image-preheat 相关端点（{len(flat)} 个）：\n")
...
def main():
    ap = argparse.ArgumentParser(
        description="suanli-image-preheat Apifox 文档实时查询（read-through）")
    ...
    p.add_argument("key", help="接口路径或短名（如 image-preheat-create）")
